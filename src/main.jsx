...
// --- PRODUCTION CONFIG ---
// Safe access to environment variables for compatibility with various target environments
const getApiKey = () => {
  try {
    // Attempt to access Vite environment variables safely
...
const App = () => {
  ...
  const handleSearch = async (e) => {
    ...
    // Replace this logic with your real MongoDB API fetch later
    setTimeout(() => {
  ...
  const getAIAnalysis = async (student) => {
    ...
    if (!key) {
      setError("AI Key not configured. Please check your environment variables.");
      setAnalyzing(false);
      return;
    }

    const prompt = `Student: ${student.d.nam}. Results: ${JSON.stringify(student.r)}. Z-Score: ${student.d.zsc}. 
    Provide a 3-sentence encouraging career and university advice for this Sri Lankan student.`;
    ...
    try {
      ...
      setError("AI Limit Reached. Use your results to check university handbooks.");
    } finally {
      setAnalyzing(false);
    }
  };
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
          ...
          <div className="px-4 py-2 bg-white rounded-full border border-slate-200 shadow-sm flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Global Edge Ready</span>
          </div>
        </div>
        ...
        <div className="bg-white p-2 rounded-3xl shadow-2xl mb-10 border border-slate-100">
          <form onSubmit={handleSearch} className="flex items-center">
            ...
            <input
              type="text"
              placeholder="Enter Index (Try 1000012)"
          ...
          <div className="lg:col-span-4 space-y-4">
            {results.map(s => (
              <div 
                ...
                className={`p-6 rounded-3xl cursor-pointer transition-all border-2 ${selectedStudent?.id === s.id ? 'bg-indigo-600 border-indigo-600 text-white shadow-xl' : 'bg-white border-transparent'}`}
              >
            ...
            {selectedStudent ? (
              <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-50">
                <div className="p-8 md:p-12 bg-slate-900 text-white">
                  ...
                  <h2 className="text-3xl font-black uppercase mb-6">{selectedStudent.d.nam}</h2>
              ...
              <div className="h-64 flex flex-col items-center justify-center bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200 text-slate-400">
                <Search className="opacity-20 mb-2" />
                <p className="text-sm font-bold">Select a result to analyze</p>
              </div>
            )}
          </div>
        </div>
